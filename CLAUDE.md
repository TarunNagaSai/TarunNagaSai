...
Adding a project = append to `lib/projects.ts` + create `content/projects/<slug>.mdx`. Adding an article = append to `lib/articles.ts` only.

### Routing

`app/` (App Router). Routes: `/`, `/about`, `/contact`, `/work`, `/work/[slug]`, `/writing`, `/not-found`. `app/work/[slug]/page.tsx` uses `generateStaticParams()` over `projects[]` so every slug is prerendered at build time. The dynamic page tries to import the MDX module by slug and falls back to a "coming soon" message if the file is absent — case-study MDX is **optional**, the project listing is canonical.
...
- `home/` — landing-page sections (`Hero`, `AboutSnippet`, `SelectedWork`, `FeaturedWriting`, `ContactStrip`). These pull from `lib/site.ts`, `lib/projects.ts`, `lib/articles.ts`.
- `work/` — `ProjectCard` and `CaseStudyHeader`.
- `ui/` — small primitives (`Tag`, `ArrowLink`, `Reveal` — Framer Motion fade-up wrapper).
